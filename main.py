...
def runAll(init_d=None, save_extra=False, sub_dir=SUB_DIR):
	"""
	...
	file_name = 'Data_' + str(end_time)[:-5].replace(':', '-').replace(' ', '_')
	...
	if os.path.isdir(sub_dir):	   # Checking if dir exists
		print('SUB_DIR')
		# print('Saving the file to (sub): ',sub_dir,'/',file_name,'.csv', sep='')
		gen_data.to_csv(sub_dir + '/' + file_name + '.csv')
	else:
		# print('Saving the file to: (same) ' + file_name + '.csv')
		print('Same dir')
		gen_data.to_csv(file_name + '.csv')
		# gen_data.to_csv('This-is-a-Test.csv')

...
P_PERM_NA = 1e-8  # Permeability of sodium ions (Na+) through the paracellular. ------ m/s
P_PERM_K = 3.4e-10  # Permeability of potassium ions (K+) through the P. ------ m/s
P_PERM_CL = 3.9e-8  # Permeability chloride ions (Cl-) through the paracellular. ------ m/s
...
A_CONS_NCL = 40.8  # Apical concentration on non-Chloride anions ------ mM
A_CONS_IO = 8.0  # Concentration of impermeable osmolytes in the apical compartment. ------ mM (milli-moles / litter)
C_CONS_NCL = 77.6  # Cellular concentration on non-Chloride anions ------ mM
C_CONS_IO = 29.0  # Concentration of impermeable osmolytes in the cellular compartment. ------ mM (milli-moles / litter)
...
J_Pump_max = 1.5e-6  # Max flow of the Na-K-ATPase pump. ------ milli-mol m^-2 s^-1
...
CACC_PERM_MAX = 4.8e-9  # The estimated MAX permeability of the CaCC Channel. ------ m / s
CACC_ATP_AT_HALF_PERM = 8.1e-5  # [ATP] when half receptors occupied (half perm). ------ muM

CFTR_PERM_MAX = 9.4e-8  # Similar concept, with the same units.
CFTR_ADO_AT_HALF_PERM = 0.18  # Similar concept, with the same units.
CFTR_ATP_AT_HALF_PERM = 6.8e-6  # Similar concept, with the same units.

BK_PERM_MAX = 7.3e-10  # Similar concept, with the same units.
BK_ATP_AT_HALF_PERM = 1.3e-3  # Similar concept, with the same units.

ENAC_PERM_MAX = 8.3e-9  # Similar concept, with the same units.
ENAC_ATP_AT_HALF_PERM = 7.2e-2  # Similar concept, with the same units.
ENAC_ADO_AT_HALF_PERM = 0.31  # Similar concept, with the same units.

CAKC_PERM_MAX = 5.7e-8  # Similar concept, with the same units.
CAKC_ATP_AT_HALF_PERM = 2.1e-4  # Similar concept, with the same units.
